...
export default function PressShowcase() {
  return (
    <section className="py-8">
      {/* same container as projects/status */}
      <div className="mx-auto max-w-5xl px-4">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-xl">in the news</h2>
          {/* optional see-all link; remove if unwanted */}
          {/* <a href="/press" className="link-underline text-sm text-muted hover:text-[color:var(--color-accent)]">see all</a> */}
        </div>
          ...
          {pressItems.map((item, i) => (
            <CardMotion key={i} maxTiltDeg={5} scale={1.02} className="rounded-2xl">
              <a
                ...
                className="card-focusable group relative flex min-h-[170px] items-stretch overflow-hidden rounded-2xl border border-neutral-800/70 bg-neutral-950/80 text-left text-neutral-100 shadow-[0_0_20px_rgba(0,0,0,0.25)] backdrop-blur-md"
              >
